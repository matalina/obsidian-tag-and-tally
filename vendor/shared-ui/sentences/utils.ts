...
export const SENTENCE_TEMPLATES: Record<string, string> = {
    ...
    "Creature": "**{creature-name-prefix} {creature-name-[creature-type]}** ({creature-disposition} {creature-descriptor} {creature-type}): This creature {creature-motivation} and attacks with {creature-attack-choice-[damage-type]} for {damage-type} damage. It features a unique ability to {creature-special-ability-[creature-type]}. It {creature-strength-[creature-type]} and {creature-weakness-[creature-type]}.",
    "Quest": "{quest-type} {quest-objective-[quest-type]}",
    "NPC/Character": "**Name** is a {(theme)-descriptor} {(theme)-species} {(theme)-type-[option]} who {(theme)-does-something}. They are motivated {character-motivation} and have a goal {character-goal}. In combat, they deal {wound-damage} damage with {character-attack-choice}, but they {character-secret}.",
    "Species": "This species is a {character-species-aged} {character-species-descriptor} people who {character-species-cultural-trait} and {character-species-feature}.",
    "Type": "This type is a {fantasy-descriptor} {fantasy-type} who {fantasy-does-something}.",
    "Wounds": "This {wound-damage-type} {wound-injury-type} is a {wound-descriptor} {wound-type-of-mark} located on {wound-body-part} that {wound-detriment}.",
    "Blood Magic Trauma": "This wound is a {wound-blood-magic-descriptor} {wound-blood-magic-mark} located on {wound-blood-magic-body-part} that {wound-blood-magic-detriment}.",
    "Faction": "**{(theme)-faction-name-pattern}** is a {(theme)-faction-descriptor} {(theme)-faction-organization-type} that {(theme)-faction-core-function} and {(theme)-faction-goal}. Despite their outward reputation, they {(theme)-faction-secret}.",
    "Location": "This location is a {location-descriptive} {location-type} that {location-distinctive-feature}",
    "Dungeon": "**{(theme)-dungeon-name-pattern}** is a {(theme)-dungeon-descriptive} {(theme)-dungeon-type} that {(theme)-dungeon-distinctive-feature}",
    ...
    "Weapon": "This weapon is a {weapon-descriptor} {material-weapon} {weapon-type} that does {damage-type} damage and {weapon-special}.",
    "Trap": "This trap is a {trap-descriptor} {trap-type} that {trigger-type} and {trap-effect}.",
    "Consumable": "This consumable is a {consumable-descriptor} {consumable-type} that {consumable-effect} and {consumable-special}."
};

// True if ref is any Faction sentence part (name, descriptor, org-type, etc.) — show as plain text, not in brackets
function isFactionSentenceRef(ref: string): boolean {
    if (ref === 'faction-name-pattern' || ref.endsWith('-faction-name-pattern')) return true;
    if (ref.startsWith('faction-name-') || ref.includes('-faction-name-')) return true;
    if (ref === 'faction-descriptor' || ref.endsWith('-faction-descriptor')) return true;
    if (ref === 'faction-organization-type' || ref.endsWith('-faction-organization-type')) return true;
    if (ref === 'faction-core-function' || ref.endsWith('-faction-core-function')) return true;
    if (ref === 'faction-goal' || ref.endsWith('-faction-goal')) return true;
    if (ref === 'faction-secret' || ref.endsWith('-faction-secret')) return true;
    return false;
}

// True if ref is any Dungeon sentence part — show as plain text, not in brackets
function isDungeonSentenceRef(ref: string): boolean {
    if (ref === 'dungeon-name-pattern' || ref.endsWith('-dungeon-name-pattern')) return true;
    if (ref.startsWith('dungeon-name-') || ref.includes('-dungeon-name-')) return true;
    if (ref === 'dungeon-descriptive' || ref.endsWith('-dungeon-descriptive')) return true;
    if (ref === 'dungeon-type' || ref.endsWith('-dungeon-type')) return true;
    if (ref === 'dungeon-distinctive-feature' || ref.endsWith('-dungeon-distinctive-feature')) return true;
    return false;
}

// True if ref is any Lair sentence part — show as plain text, not in brackets
function isLairSentenceRef(ref: string): boolean {
    if (ref === 'lair-name-pattern') return true;
    if (ref.startsWith('lair-name-') || ref.includes('-lair-name-')) return true;
    if (ref === 'lair-descriptive' || ref.endsWith('-lair-descriptive')) return true;
    if (ref === 'lair-type' || ref.endsWith('-lair-type')) return true;
    if (ref === 'lair-distinctive-feature' || ref.endsWith('-lair-distinctive-feature')) return true;
    return false;
}
...
export function resolveTableReference(
    ...
    if (tableRef.startsWith('creature-attack-choice-') || 
    ...
    if (tableRef.includes('[') && tableRef.includes(']')) {
        ...
        if (match) {
            ...
            if (category === 'creature-attack-choice' || 
...
export function generateFactionName(theme: string = "fantasy"): string {
    ...
    try {
        ...
        // Keep resolving until no refs remain (reset lastIndex after each replace so we don't skip refs)
        const refRegex = /\{(?:modern-|fantasy-|monster-hunter-)?faction-name-([^}]+)\}/g;
        let match;
        let iterations = 0;
        const maxIterations = 20; // prevent infinite loops from bad data
        while ((match = refRegex.exec(pattern)) !== null && iterations < maxIterations) {
            iterations++;
            ...
            refRegex.lastIndex = 0; // re-scan so we don't skip remaining refs
        }
        return pattern;
    } catch (error) {
...
export function generateLairName(): string {
    ...
    try {
        ...
        // Keep resolving until no refs remain (find one match, replace, re-scan from start)
        while (iterations < maxIterations) {
            refRegex.lastIndex = 0;
            const match = refRegex.exec(pattern);
            if (!match) break;
            iterations++;
...
// Generate a dungeon name from pattern table (theme e.g. fantasy, modern)
export function generateDungeonName(theme: string = "fantasy"): string {
    const store = getTableStore();
    const themeLower = theme.toLowerCase();
    
    try {
        const patternTable = store.hasTable(`${themeLower}-dungeon-name-pattern`)
            ? `${themeLower}-dungeon-name-pattern`
            : 'dungeon-name-pattern';
        const patternResult = store.random(patternTable);
        let pattern = patternResult.result;
        
        // Replace {theme-dungeon-name-type}, {theme-dungeon-name-threat}, etc. with actual values
        const refRegex = /\{([^}]*dungeon-name-[^}]+)\}/g;
        let match;
        let iterations = 0;
        const maxIterations = 20;
        while ((match = refRegex.exec(pattern)) !== null && iterations < maxIterations) {
        ...
        console.error('Error generating dungeon name:', error);
        return 'Name';
    }
}
...
export function generateSentence(
    ...
    for (const ref of independentRefs) {
        ...
        if (tableName === '__TYPE_FOR_CHARACTER__' || tableName.toLowerCase() === '__type_for_character__' || 
        ...
        try {
            ...
            if (ref.ref === 'faction-name-pattern' || ref.ref.endsWith('-faction-name-pattern')) {
                ...
                if (ref.ref.endsWith('-faction-name-pattern')) {
                    ...
                    if (themeMatch) nameTheme = themeMatch[1];
                }
            ...
            // Special handling for dungeon-name-pattern: generate full name with pattern resolution
            if (ref.ref === 'dungeon-name-pattern' || ref.ref.endsWith('-dungeon-name-pattern')) {
                let nameTheme = theme;
                if (ref.ref.endsWith('-dungeon-name-pattern')) {
                    const themeMatch = ref.ref.match(/^(.+?)-dungeon-name-pattern$/);
                    if (themeMatch) nameTheme = themeMatch[1];
                }
                const dungeonName = generateDungeonName(nameTheme);
                replacements.set(ref.index, dungeonName);
                resolvedValues.set(ref.ref, dungeonName);
                continue;
            }
            ...
            if (tableName === '__WOUND_DETRIMENT__') {
                try {
                    ...
                    const validDamageTypes = ['Physical', 'Psychic', 'Elemental', 'Necrotic', 'Radiant', 'Shadow', 'Chaos', 'Toxic'];
                    const injuryTypes = ['Strain', 'Lingering Trauma', 'Debilitating Injury', 'Lasting Scar'];
                    ...
                    if (damageType && (damageType.includes(' ') || injuryTypes.includes(damageType))) {
                        ...
                        console.warn(`Invalid damage type detected (looks like injury type): ${damageType}. Rolling new damage type.`);
                    ...
                    if (!injuryTypeDisplay) {
                        ...
                        injuryTypeDisplay = severity.split('-').map(word => 
            ...
            if (tableName === '__WOUND_DAMAGE_TYPE__') {
                ...
                if (storedDamageType && (storedDamageType.includes(' ') || injuryTypes.includes(storedDamageType))) {
                    // This is an injury type, not a damage type - clear it and roll a new one
                    console.warn(`Invalid damage type detected (looks like injury type): ${storedDamageType}. Rolling new damage type.`);
            ...
            if (tableName === '__WOUND_INJURY_TYPE__') {
                ...
                if (!storedInjuryType) {
                    ...
                    try {
                        ...
                        const injuryTypeDisplay = severity.split('-').map(word => 
                            word.charAt(0).toUpperCase() + word.slice(1)
                        ).join(' ');
                        resolvedValues.set('__wound_injury_type__', injuryTypeDisplay);
            ...
            const isFactionRef = isFactionSentenceRef(ref.ref);
            const isDungeonRef = isDungeonSentenceRef(ref.ref);
            const isLairRef = isLairSentenceRef(ref.ref);
            
            if (isCreatureName || isFactionRef || isDungeonRef || isLairRef) {
                // do nothing (plain text, no brackets)
            } else {
            ...
            const valueForLookup = (isCreatureName || isFactionRef || isDungeonRef || isLairRef) ? tableResult.result : tableResult.result.toLowerCase();
    ...
    for (const ref of dependentRefs) {
        ...
        const hasTypeOptionPattern = ref.ref.includes('type') && ref.ref.includes('[option]') && ref.ref.endsWith('[option]');
        const isCharacterSentence = template.includes('**Name**') || npcOption !== undefined;
        const npcOptionNotSet = !npcOption || npcOption === '';
        ...
        try {
            ...
            const isTypeForCharacter = (tableName === '__TYPE_FOR_CHARACTER__' || tableName.toLowerCase() === '__type_for_character__' ||
    ...
    result = result.replace(/([.!?]\s+)([a-z])/g, (_match, punc, letter) => punc + letter.toUpperCase());
...
export function generateSentenceWithTags(
    ...
    for (const ref of independentRefs) {
        ...
        // Check for __TYPE_FOR_CHARACTER__ BEFORE any other processing
        // This must be checked before the error handling below
        const isCharacterSentence = (template.includes('**Name**') && npcOption !== undefined) || npcOption !== undefined;
        if (tableName === '__TYPE_FOR_CHARACTER__' || tableName.toLowerCase() === '__type_for_character__' || 
        ...
        try {
            ...
            const isTypeForCharacter = tableName === '__TYPE_FOR_CHARACTER__' || 
            ...
            if (tableName.startsWith('[Table not found:') || tableName.startsWith('[Dependent table not found:')) {
                ...
                const validTagTypes = [
                    'umbrella', 'special', 'species', 'type', 'aged',
                    'skill', 'ability', 'attack',
                    'condition', 'reputation',
                    'background', 'experience', 'language', 'goal', 'secret', 'location', 'spell', 'item', 'use',
                    'flaw', 'weakness', 'damage', 'wound',
                    'scene',
                    'trait', 'strong', 'descriptor', 'drive'
                ];
            ...
            if (isCreatureName || isFactionRef || isDungeonRef || isLairRef) {
                // Keep as is (plain text, no brackets)
            } else {
                value = value.toLowerCase();
                value = `[${value}]`;
            }
    ...
    for (const ref of dependentRefs) {
        ...
        if (ref.ref.startsWith('creature-special-ability-')) {
            try {
                ...
                if (value.toLowerCase() === creatureType.toLowerCase() || 
                    value.toLowerCase() === normalizedCreatureType) {
        ...
        try {
            ...
            if (tableName === '__TYPE_FOR_CHARACTER__' || 
                tableName.toLowerCase() === '__type_for_character__' ||
                tableName.includes('__type_for_character__') ||
